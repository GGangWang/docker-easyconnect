...
## 确定策略路由方式
ip rule add iif lo table 2 sport 1081
...
elif iptables -t mangle -A OUTPUT -j MARK --set-mark 1 -p tcp --sport 1081 2>/dev/null ; then
	iptables -t mangle -D OUTPUT -j MARK --set-mark 1 -p tcp --sport 1081
...
ip rule del iif lo sport 1081 table 2
